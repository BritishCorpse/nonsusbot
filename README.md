...
If multiple options pass, a `CommandUsageError`{:.javascript} error will be thrown.
...
If checks is undefined or is empty, the option will always pass.

A check can be inverted by replacing the value with `{not: value}`{:.javascript}. See [Not](#not).
...
All checks can be inverted by replacing the value with `{not: value}`{:.javascript}.

**Note:** when a check is inverted, the option will pass if the argument is not given.
To disable this behavior, add the `isempty: {not: null}`{:.javascript} check.
